...
# Create segments
data <- data |>
  mutate(segment =ntile(data$apr_sph, 4))
...
## Difference-in-Differences (productivity segment)
```{r}

# Use Panel Data Estimators to evaluate differences in mean within units between the two periods

panel_df2 <- pdata.frame(kU_2x2_DD, index=c('segment', 'time'))

standard_did2 <- plm(sales_per_hr ~ assignment,
                    data = panel_df2,
                    model = 'within',
                    effect = 'twoways')

standard_did2 |> summary()
...
```{r fect_plot}
plot(fect_est, count = FALSE, return.test = TRUE) # returns plots and test statistics
...
```{r}
# Create a dataframe for use in comparing segment data 
control_change <- data |>
  filter(assignment == 0) |>
  group_by(Type = segment) |>
  mutate(change_rate = may_sph/apr_sph) |>
  summarize(Percent_change = mean(change_rate)*100)
...
```{r}
# Create a dataframe for use in comparing segment data 
mean_changes <- function(df, var) { df %>% 
    group_by(segment, month) %>% 
    summarise( mean_val = mean(.data[[var]], na.rm = TRUE), .groups = "drop" ) %>% 
    pivot_wider( names_from = month, values_from = mean_val, names_prefix = "mean_" ) %>%
    mutate(change = (mean_may - mean_apr) * 100) %>% 
    select(segment, change) }

mean_changes(data, rts)
```
